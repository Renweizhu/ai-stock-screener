      ...
      (function() {
        ...
        function App() {
          ...
          }, [currentStocks, portfolioBudget, portfolioStrategy]);
          ...
          const handleSend = async (textOverride) => {
            ...
            try {
              const res = await fetch('/api/chat', {
                ...
                body: JSON.stringify({ prompt: rawQuery, history: messages })
              });
              if (res.ok) {
                const data = await res.json();
                if (data.mode === 'gemini_contextual_screener' && data.reply) {
                  setMessages(prev => [...prev, {
                    ...
                    badge: "✨ Gemini 多輪對話記憶引擎",
                    portfolioCard: data.portfolioCard || inlinePortfolioCard
                  }]);
            ...
            setMessages(prev => [...prev, { sender: 'ai', text: replyText, topStocks: top3, badge: badgeTag, portfolioCard: inlinePortfolioCard }]);
            setIsProcessing(false);
          };
              ...
              h('div', { className: 'glass-panel', style: { padding: '1.1rem 1.5rem', marginBottom: '1.5rem', background: 'linear-gradient(135deg, rgba(15, 23, 42, 0.9) 0%, rgba(30, 41, 59, 0.8) 100%)', border: '1px solid rgba(16, 185, 129, 0.3)' } },
                h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.75rem' } },
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.6rem' } },
                    ...
                    h('div', null,
                      h('h2', { className: 'gradient-text', style: { fontSize: '1.15rem', fontWeight: 800 } }, '全球熱門飆股與焦點標的 (Featured Hot Stocks)'),
                      h('p', { style: { fontSize: '0.75rem', color: 'var(--text-secondary)' } }, '即時監控台美雙市場熱搜、高殖利率霸王與 AI 算力龍頭')
                    )
                  ),
                  // Hot Category Tabs
              ...
              h('div', { className: `floating-chat-widget ${isChatMinimized ? 'minimized' : ''}` },
                h('div', {
                  ...
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.5rem' } },
                    ...
                    h('strong', { style: { fontSize: '0.9rem', color: 'var(--text-primary)' } }, 'StockMind AI 投資特助'),
                    h('span', { className: 'pill-badge pill-purple', style: { fontSize: '0.65rem' } }, '智能選股助手')
                  ),
                  h('button', {
