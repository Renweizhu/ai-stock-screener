...
<html lang="zh-TW">
  <head>
    ...
    <title>StockMind AI - 全台股即時對話篩選與投資組合配置系統</title>
  ...
  <body>
    <div id="root">
      <div style="padding: 2rem; text-align: center; color: #10b981; font-family: sans-serif;">
        <h2>📈 StockMind AI 載入中...</h2>
      </div>
    </div>
    ...
    <script>
      window.onerror = function(msg, url, lineNo, columnNo, error) {
        ...
        if (root) {
          root.innerHTML = '<div style="padding: 2rem; background: #1e1b4b; color: #f87171; border-radius: 12px; margin: 2rem; font-family: sans-serif;">' +
            '<h3>⚠️ 診斷提示</h3><p>' + msg + ' (Line ' + lineNo + ')</p></div>';
        }
        return false;
      };
    </script>
    ...
    <script>
      (function() {
        ...
        function App() {
          ...
          const [lastSync, setLastSync] = useState("即時連線中");
          ...
          const fetchLiveStocks = async () => {
            try {
              ...
              if (res.ok) {
                ...
                if (data.status === 'success' && Array.isArray(data.stocks) && data.stocks.length > 0) {
                  if (data.last_sync) setLastSync(data.last_sync);
                  setStockCount(data.stocks.length);

                  const newFlashMap = {};
                  data.stocks.forEach(newS => {
                    const oldS = currentStocks.find(s => s.ticker === newS.ticker);
                    if (oldS && oldS.price !== newS.price) {
                      newFlashMap[newS.ticker] = newS.price > oldS.price ? 'flash-up' : 'flash-down';
                    }
                  });
                  ...
                  if (Object.keys(newFlashMap).length > 0) {
                    ...
                    setTimeout(() => setPriceFlashMap({}), 1000);
                  }
                }
              }
            } catch (e) {}
          ...
          useEffect(() => {
            fetchLiveStocks();
            const interval = setInterval(fetchLiveStocks, 3000);
          ...
          const getFeaturedHotStocks = (stocks, category) => {
            ...
            if (category === 'qualityCompany') {
              return [...stocks].filter(s => s.roe >= 10.0 && s.yield >= 2.5).sort((a, b) => b.roe - a.roe).slice(0, 6);
            }
            if (category === 'gainers') {
          ...
          const [globalIndexTab, setGlobalIndexTab] = useState('ALL');
          ...
          const [backtestBudget, setBacktestBudget] = useState(1000000);
          const [backtestStrategy, setBacktestStrategy] = useState('lump_sum');
          const [backtestMonthlyAmount, setBacktestMonthlyAmount] = useState(10000);
          const [backtestDrip, setBacktestDrip] = useState(true);
          const [backtestBenchmark, setBacktestBenchmark] = useState('0050');
          ...
          const runPortfolioBacktest = async (
            ...
            const targetItems = (items !== null && Array.isArray(items) && items.length > 0) 
              ? items 
              : (backtestItems && backtestItems.length > 0 
                  ? backtestItems 
                  : (activePortfolioItems && activePortfolioItems.length > 0 
                      ? activePortfolioItems 
                      : [{ ticker: '0050', name: '元大台灣50', price: 104.6, roe: 15.2, yield: 3.5, weight: 1.0 }]));
            const targetYears = years !== null ? years : backtestYears;
            const targetBudget = budget !== null ? budget : backtestBudget;
            const targetStrategy = strategy !== null ? strategy : backtestStrategy;
            const targetMonthly = monthlyAmt !== null ? monthlyAmt : backtestMonthlyAmount;
            const targetDrip = dripVal !== null ? dripVal : backtestDrip;
            const targetBenchmark = bmVal !== null ? bmVal : backtestBenchmark;

            setBacktestItems(targetItems);
            setBacktestYears(targetYears);
            setBacktestBudget(targetBudget);
            setBacktestStrategy(targetStrategy);
            setBacktestMonthlyAmount(targetMonthly);
            setBacktestDrip(targetDrip);
            setBacktestBenchmark(targetBenchmark);
          ...
          return h('div', { className: 'app-container' },
              ...
              h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' } },
                h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.75rem' } },
                  ...
                  h('div', null,
                    ...
                    h('div', { style: { display: 'flex', gap: '0.4rem', background: 'rgba(255,255,255,0.05)', padding: '3px', borderRadius: '10px', marginTop: '0.45rem' } },
                      h('button', {
                        onClick: () => setActiveView('screener'),
                        style: {
                          background: activeView === 'screener' ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)' : 'transparent',
                          color: activeView === 'screener' ? '#090d16' : 'var(--text-secondary)',
                          border: 'none', padding: '0.35rem 0.9rem', borderRadius: '8px', fontSize: '0.82rem', fontWeight: 800, cursor: 'pointer'
                        }
                      }, '📊 股票與 ETF 篩選主控台'),
                      h('button', {
                        onClick: () => setActiveView('macro'),
                        style: {
                          background: activeView === 'macro' ? 'linear-gradient(135deg, #06b6d4 0%, #3b82f6 100%)' : 'transparent',
                          color: activeView === 'macro' ? '#fff' : 'var(--text-secondary)',
                          border: 'none', padding: '0.35rem 0.9rem', borderRadius: '8px', fontSize: '0.82rem', fontWeight: 800, cursor: 'pointer'
                        }
                      }, '🌐 全球總體經濟指數與指標')
                    )
                ...
                h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.75rem' } },
                  ...
                  h('button', { onClick: () => runPortfolioBacktest(activePortfolioItems, 3, portfolioBudget), className: 'btn-secondary', style: { fontSize: '0.82rem', border: '1px solid rgba(6, 182, 212, 0.6)', background: 'linear-gradient(135deg, rgba(6, 182, 212, 0.25) 0%, rgba(59, 130, 246, 0.25) 100%)', color: '#38bdf8', fontWeight: 800 } }, '📈 歷史回測 (Backtest)'),
            ...
            // Main Layout (Screener View vs Macro View)
            activeView === 'macro' ?
              // Dedicated Macro Economic Dashboard Page
              h('div', { className: 'glass-panel', style: { padding: '1.5rem', minHeight: '80vh', marginBottom: '1.5rem' } },
                // Macro Header
                h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '1rem', flexWrap: 'wrap', gap: '1rem' } },
                  h('div', null,
                    h('h2', { className: 'gradient-text', style: { fontSize: '1.4rem', fontWeight: 800 } }, '🌐 全球總體經濟與指標主控台 (Macroeconomic Dashboard)'),
                    h('p', { style: { fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.2rem' } }, '即時追蹤美聯儲利率、CPI 通膨、美債殖利率、恐慌指數 VIX 與台灣景氣燈號')
                  ),
                  h('button', { onClick: fetchMacroData, className: 'btn-primary', style: { fontSize: '0.8rem', padding: '0.4rem 1rem' } }, '🔄 即時連線刷新數據')
                ),

                // 0. Global Major Stock Market Indices & Sparkline Charts Section
                h('div', { style: { marginBottom: '1.75rem', background: 'rgba(15, 23, 42, 0.85)', padding: '1.25rem', borderRadius: '12px', border: '1px solid rgba(6, 182, 212, 0.4)' } },
                  h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.75rem' } },
                    h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.5rem' } },
                      ...
                      h('div', null,
                        h('h3', { className: 'gradient-text', style: { fontSize: '1.15rem', fontWeight: 800 } }, '全球主要股市指數行情與動態走勢圖 (Global Stock Market Indices)'),
                        h('p', { style: { fontSize: '0.75rem', color: 'var(--text-secondary)' } }, '即時追蹤美股四大指數、台股加權、日韓與歐洲主要國家股市指數')
                      )
                    ),
                    h('div', { style: { display: 'flex', gap: '0.3rem', background: 'rgba(255,255,255,0.06)', padding: '3px', borderRadius: '8px', flexWrap: 'wrap' } },
                      [
                        { id: 'ALL', label: '🌐 全部股市' },
                        { id: 'US', label: '🇺🇸 美股四大指數' },
                        { id: 'TW', label: '🇹🇼 台股加權' },
                        ...
                        h('button', {
                          key: tab.id,
                          onClick: () => setGlobalIndexTab(tab.id),
                          style: {
                            background: globalIndexTab === tab.id ? 'var(--accent-cyan)' : 'transparent',
                            color: globalIndexTab === tab.id ? '#090d16' : 'var(--text-secondary)',
                            border: 'none', padding: '0.25rem 0.65rem', borderRadius: '6px', fontSize: '0.75rem', fontWeight: globalIndexTab === tab.id ? 800 : 500, cursor: 'pointer'
                          }
                        }, tab.label)
                      )
                    )
                  ...
                  // Global Indices Grid Cards with Dynamic SVG Sparklines
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: '0.85rem' } },
                    ((macroData && macroData.globalIndices) || []).filter(idxItem => {
                      if (globalIndexTab === 'ALL') return true;
                      if (globalIndexTab === 'TW') return idxItem.region === 'TW';
                      if (globalIndexTab === 'US') return idxItem.region === 'US';
                      if (globalIndexTab === 'ASIA') return idxItem.region === 'ASIA' || idxItem.region === 'TW';
                      if (globalIndexTab === 'EU') return idxItem.region === 'EU';
                      return true;
                    }).map((idxItem, keyIdx) =>
                      h('div', { key: keyIdx, className: 'glass-panel-interactive', style: { padding: '0.85rem', borderTop: `3px solid ${idxItem.isUp ? '#10b981' : '#ef4444'}` } },
                        h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' } },
                          h('div', { style: { fontSize: '0.82rem', fontWeight: 800, color: '#fff' } }, `${idxItem.flag} ${idxItem.name}`),
                          h('span', { style: { fontSize: '0.7rem', color: idxItem.isUp ? '#10b981' : '#ef4444', fontWeight: 800 } }, idxItem.changePct)
                        ),
                        h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.4rem' } },
                          h('div', { style: { fontSize: '1.2rem', fontWeight: 900, color: idxItem.isUp ? '#34d399' : '#fb7185' } }, idxItem.price.toLocaleString()),
                          h('div', { style: { fontSize: '0.72rem', color: 'var(--text-muted)' } }, idxItem.change)
                        ),
                        // Mini SVG Sparkline Trend Chart
                        h('svg', { viewBox: '0 0 180 40', style: { width: '100%', height: '35px', overflow: 'visible' } },
                          (() => {
                            const hist = idxItem.history || [];
                            if (hist.length < 2) return null;
                            const maxVal = Math.max(...hist);
                            const minVal = Math.min(...hist);
                            const range = maxVal - minVal || 1;
                            const points = hist.map((v, i) => `${(i / (hist.length - 1)) * 170 + 5},${35 - ((v - minVal) / range) * 28 + 3}`).join(' ');

                            return h('polyline', {
                              fill: 'none',
                              stroke: idxItem.isUp ? '#10b981' : '#ef4444',
                              strokeWidth: '2.5',
                              points: points
                            });
                          })()
                ...
                // Section 0.5: Global Foreign Exchange Rates Hub
                h('div', { style: { marginBottom: '1.8rem' } },
                  h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.85rem' } },
                    h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.5rem' } },
                      h('span', { style: { fontSize: '1.2rem' } }, '💱'),
                      h('h3', { style: { fontSize: '1.05rem', fontWeight: 800, color: '#f59e0b' } }, '全球主要國家貨幣即時匯率看板 (Global Live FX Rates)')
                    ),
                    h('span', { className: 'pill-badge pill-gold', style: { fontSize: '0.72rem' } }, '對新台幣 (TWD) 實時連線')
                  ),
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.85rem' } },
                    ((macroData && macroData.fxRates) || [
                      { flag: "🇺🇸", code: "USD/TWD", name: "美元 / 新台幣", rate: 32.45, changePct: "-0.12%", isUp: false, history: [31.8, 32.2, 32.5, 32.45] },
                      { flag: "🇯🇵", code: "JPY/TWD", name: "日圓 / 新台幣", rate: 0.2185, changePct: "+0.45%", isUp: true, history: [0.205, 0.212, 0.2185] },
                      { flag: "🇪🇺", code: "EUR/TWD", name: "歐元 / 新台幣", rate: 35.42, changePct: "+0.18%", isUp: true, history: [34.5, 35.0, 35.42] },
                      { flag: "🇬🇧", code: "GBP/TWD", name: "英鎊 / 新台幣", rate: 41.85, changePct: "+0.22%", isUp: true, history: [40.5, 41.2, 41.85] },
                      { flag: "🇨🇳", code: "CNY/TWD", name: "人民幣 / 新台幣", rate: 4.52, changePct: "-0.05%", isUp: false, history: [4.45, 4.50, 4.52] },
                      { flag: "🇰🇷", code: "KRW/TWD", name: "韓元 / 新台幣", rate: 0.0238, changePct: "+0.15%", isUp: true, history: [0.023, 0.0235, 0.0238] },
                      { flag: "🇦🇺", code: "AUD/TWD", name: "澳幣 / 新台幣", rate: 21.65, changePct: "+0.35%", isUp: true, history: [20.8, 21.4, 21.65] },
                      { flag: "🇸🇬", code: "SGD/TWD", name: "新加坡幣 / 新台幣", rate: 24.28, changePct: "+0.10%", isUp: true, history: [23.8, 24.1, 24.28] }
                    ]).map((fx, fxIdx) =>
                      h('div', { key: fxIdx, className: 'glass-panel-interactive', style: { padding: '0.8rem', borderLeft: `4px solid ${fx.isUp ? '#10b981' : '#ef4444'}` } },
                        h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.2rem' } },
                          h('div', { style: { fontSize: '0.8rem', fontWeight: 800, color: '#fff' } }, `${fx.flag} ${fx.name}`),
                          h('span', { style: { fontSize: '0.7rem', color: fx.isUp ? '#10b981' : '#ef4444', fontWeight: 800 } }, fx.changePct)
                        ),
                        h('div', { style: { fontSize: '1.25rem', fontWeight: 900, color: '#f59e0b', margin: '0.2rem 0 0.4rem', fontFamily: 'var(--font-mono)' } },
                          `${fx.rate} 元`
                        ),
                        // Mini SVG Sparkline
                        h('svg', { viewBox: '0 0 160 30', style: { width: '100%', height: '28px', overflow: 'visible' } },
                          (() => {
                            const hist = fx.history || [fx.rate * 0.98, fx.rate];
                            const maxVal = Math.max(...hist);
                            const minVal = Math.min(...hist);
                            const range = maxVal - minVal || 1;
                            const points = hist.map((v, i) => `${(i / (hist.length - 1)) * 150 + 5},${25 - ((v - minVal) / range) * 20 + 2}`).join(' ');

                            return h('polyline', {
                              fill: 'none',
                              stroke: fx.isUp ? '#10b981' : '#ef4444',
                              strokeWidth: '2',
                              points: points
                            });
                          })()
                ...
                // 1. US Macro & Fed Policy Section
                h('div', { style: { marginBottom: '1.5rem' } },
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.85rem' } },
                    h('span', { style: { fontSize: '1.2rem' } }, '🏛️'),
                    h('h3', { style: { fontSize: '1.05rem', fontWeight: 800, color: '#38bdf8' } }, '一、 美國宏觀與貨幣政策指標 (US Macro & Fed Policy)')
                  ),
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.85rem' } },
                    [
                      { icon: '🏦', name: '美聯儲基準利率 (Fed Rate)', val: macroData ? `${macroData.fedRate.value}${macroData.fedRate.unit}` : '5.25%', status: '降息預期發酵', color: '#10b981' },
                      { icon: '📈', name: 'CPI 通膨率 (YoY)', val: macroData ? `${macroData.cpiInflation.value}${macroData.cpiInflation.unit}` : '3.0%', status: '通膨持續放緩', color: '#38bdf8' },
                      { icon: '🎯', name: '美聯儲 Core PCE 物價指數', val: macroData ? `${macroData.corePce.value}${macroData.corePce.unit}` : '2.6%', status: '央行最看重通膨標竿', color: '#c084fc' },
                      { icon: '👷', name: '非農就業人數 (NFP) & 失業率', val: macroData ? `${macroData.nfpJobless.nfpNew} / ${macroData.nfpJobless.value}%` : '+18.5萬 / 4.1%', status: '就業市場溫和軟著陸', color: '#34d399' },
                      { icon: '📝', name: '初次申請失業金人數 (Weekly)', val: macroData ? `${macroData.initialClaims.value} ${macroData.initialClaims.unit}` : '22.5 萬人', status: '高頻邊際勞工監測', color: '#fbbf24' },
                      { icon: '🏭', name: 'ISM 製造業與服務業 PMI', val: macroData ? `${macroData.ismPmi.value} ${macroData.ismPmi.unit}` : '51.6 點', status: '高於 50 榮枯線', color: '#38bdf8' },
                      { icon: '💵', name: '美債 10 年期殖利率', val: macroData ? `${macroData.us10yYield.value}${macroData.us10yYield.unit}` : '4.18%', status: '資產估值壓力減輕', color: '#c084fc' },
                      { icon: '📉', name: '美債 10Y-2Y 殖利率倒掛', val: macroData ? `${macroData.yieldInversion.spread}${macroData.yieldInversion.unit}` : '-0.17%', status: '倒掛收斂 衰退風險低', color: '#fb7185' }
                    ].map((card, idx) =>
                      h('div', { key: idx, className: 'glass-panel-interactive', style: { padding: '0.85rem 1rem', borderLeft: `4px solid ${card.color}` } },
                        h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.3rem' } },
                          h('span', { style: { fontSize: '1.1rem' } }, card.icon),
                          h('span', { className: 'pill-badge pill-cyan', style: { fontSize: '0.65rem' } }, card.status)
                        ),
                        h('div', { style: { fontSize: '0.75rem', color: 'var(--text-secondary)' } }, card.name),
                        h('div', { style: { fontSize: '1.25rem', fontWeight: 900, color: card.color, marginTop: '2px' } }, card.val)
                      )
                    )
                  )
                ),

                // 2. Taiwan Economy & AI Fundamentals Section
                h('div', { style: { marginBottom: '1.5rem' } },
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.85rem' } },
                    h('span', { style: { fontSize: '1.2rem' } }, '🇹🇼'),
                    h('h3', { style: { fontSize: '1.05rem', fontWeight: 800, color: '#fbbf24' } }, '二、 台灣本土經濟與 AI 產業基本面 (Taiwan & AI Fundamentals)')
                  ),
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '0.85rem' } },
                    [
                      { icon: '🚥', name: '國發會景氣對策信號燈號', val: macroData ? `${macroData.taiwanLightSignal.signal} (${macroData.taiwanLightSignal.score}分)` : '黃紅燈 (36分)', status: '景氣熱絡穩健', color: '#fbbf24' },
                      { icon: '🚢', name: '經濟部外銷訂單 (YoY)', val: macroData ? `${macroData.exportOrders.value} ${macroData.exportOrders.unit}` : '+12.5% YoY', status: '連三月雙位數成長', color: '#10b981' },
                      { icon: '💻', name: '電子零組件與 AI 晶片出口', val: macroData ? `${macroData.semiconductorExport.value} ${macroData.semiconductorExport.unit}` : '+18.2% YoY', status: '台積電與 AI 先導指標', color: '#34d399' },
                      { icon: '🏭', name: '台灣製造業 PMI', val: macroData ? `${macroData.taiwanPmi.value} ${macroData.taiwanPmi.unit}` : '53.2 點', status: '擴張區間續強', color: '#38bdf8' },
                      { icon: '📦', name: '新訂單 / 庫存比 (Orders/Inventory)', val: macroData ? `${macroData.orderInventoryRatio.ratio}` : '1.15', status: '健康補庫存週期 (>1.0)', color: '#c084fc' }
                    ].map((card, idx) =>
                      h('div', { key: idx, className: 'glass-panel-interactive', style: { padding: '0.85rem 1rem', borderLeft: `4px solid ${card.color}` } },
                        h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.3rem' } },
                          h('span', { style: { fontSize: '1.1rem' } }, card.icon),
                          h('span', { className: 'pill-badge pill-emerald', style: { fontSize: '0.65rem' } }, card.status)
                        ),
                        h('div', { style: { fontSize: '0.75rem', color: 'var(--text-secondary)' } }, card.name),
                        h('div', { style: { fontSize: '1.25rem', fontWeight: 900, color: card.color, marginTop: '2px' } }, card.val)
                      )
                ...
                // 3. Institutional Chips & Market Sentiment Section
                h('div', { style: { marginBottom: '1.5rem' } },
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.85rem' } },
                    h('span', { style: { fontSize: '1.2rem' } }, '📊'),
                    h('h3', { style: { fontSize: '1.05rem', fontWeight: 800, color: '#10b981' } }, '三、 市場籌碼與資金槓桿指標 (Capital Chips & Sentiment)')
                  ),
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '0.85rem' } },
                    [
                      { icon: '🏛️', name: '三大法人買賣超 (外資/投信/自營)', val: macroData ? `${macroData.threeInstitutional.totalNet} (外資 ${macroData.threeInstitutional.foreignNet})` : '+155 億 (外資 +128億)', status: '外資連續買超挺進', color: '#10b981' },
                      { icon: '⚔️', name: '台指期外資淨未平倉口數', val: macroData ? `${macroData.foreignFuturesNetPos.contracts}` : '+6,800 口', status: '多頭避險姿勢良好', color: '#38bdf8' },
                      { icon: '📉', name: '台股融資融券餘額 (Margin Debt)', val: macroData ? `${macroData.taiwanMarginDebt.value} ${macroData.taiwanMarginDebt.unit}` : '2,850 億元', status: '散戶槓桿健康未過熱', color: '#fbbf24' },
                      { icon: '📊', name: 'VIX 恐慌指數 (Volatility)', val: macroData ? `${macroData.vixIndex.value} ${macroData.vixIndex.unit}` : '14.85 點', status: '極低波動多頭行情', color: '#34d399' },
                      { icon: '🧠', name: 'CNN 貪婪與恐懼指數', val: macroData ? `${macroData.fearGreedIndex.score} (Greed)` : '68 (貪婪)', status: '市場情緒樂觀', color: '#c084fc' },
                      { icon: '💼', name: '美股 13F 機構持倉與槓桿', val: macroData ? `${macroData.us13FHoldings.status}` : '淨增持科技股', status: '對沖基金大舉做多', color: '#10b981' }
                    ].map((card, idx) =>
                      h('div', { key: idx, className: 'glass-panel-interactive', style: { padding: '0.85rem 1rem', borderLeft: `4px solid ${card.color}` } },
                        h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.3rem' } },
                          h('span', { style: { fontSize: '1.1rem' } }, card.icon),
                          h('span', { className: 'pill-badge pill-purple', style: { fontSize: '0.65rem' } }, card.status)
                        ),
                        h('div', { style: { fontSize: '0.75rem', color: 'var(--text-secondary)' } }, card.name),
                        h('div', { style: { fontSize: '1.25rem', fontWeight: 900, color: card.color, marginTop: '2px' } }, card.val)
                      )
                ...
                // Taiwan Light Signal & Fed vs CPI Chart Panel
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.25rem', marginBottom: '1.5rem' } },
                  
                  // Taiwan Business Indicators Panel
                  h('div', { className: 'glass-panel', style: { padding: '1.25rem' } },
                    h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.85rem' } },
                      h('span', { style: { fontSize: '1.3rem' } }, '🚥'),
                      h('h3', { style: { fontSize: '1rem', fontWeight: 800, color: '#fbbf24' } }, '台灣景氣對策信號燈號 (Taiwan Business Barometer)')
                    ),
                    h('div', { style: { background: 'rgba(245,158,11,0.15)', border: '1px solid #fbbf24', borderRadius: '10px', padding: '1rem', textAlign: 'center', marginBottom: '1rem' } },
                      h('div', { style: { fontSize: '0.8rem', color: 'var(--text-secondary)' } }, '當前景氣燈號得分 (36 分)'),
                      h('div', { style: { fontSize: '2rem', fontWeight: 900, color: '#fbbf24', margin: '0.3rem 0' } }, '🟡 黃紅燈 (景氣熱絡)'),
                      h('p', { style: { fontSize: '0.78rem', color: '#e2e8f0', lineHeight: 1.5 } }, '受益於半導體先進製程、AI 伺服器出口與全球科技需求強勁，國內製造業動態極佳。')
                    ),
                    h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.75rem' } },
                      h('div', { style: { background: 'rgba(255,255,255,0.03)', padding: '0.5rem', borderRadius: '6px' } }, '📈 海關出口值: 轉趨強勁'),
                      h('div', { style: { background: 'rgba(255,255,255,0.03)', padding: '0.5rem', borderRadius: '6px' } }, '🏭 機械設備進口: 爆發成長'),
                      h('div', { style: { background: 'rgba(255,255,255,0.03)', padding: '0.5rem', borderRadius: '6px' } }, '💰 貨幣總計數 M1B: 向上拉升'),
                      h('div', { style: { background: 'rgba(255,255,255,0.03)', padding: '0.5rem', borderRadius: '6px' } }, '🏢 批發零售餐飲: 穩定增長')
                    )
                  ),

                  // Fed Rate vs CPI Inflation History SVG Chart
                  h('div', { className: 'glass-panel', style: { padding: '1.25rem' } },
                    h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.85rem' } },
                      h('h3', { style: { fontSize: '1rem', fontWeight: 800, color: '#38bdf8' } }, '📉 聯準會利率 vs 美國 CPI 通膨走勢 (Fed Rate vs. CPI Inflation)'),
                      h('div', { style: { display: 'flex', gap: '0.5rem', fontSize: '0.72rem' } },
                        h('span', { style: { color: '#10b981', fontWeight: 800 } }, '🟢 Fed 利率'),
                        h('span', { style: { color: '#fb7185', fontWeight: 800 } }, '🔴 CPI 通膨')
                      )
                    ),
                    h('svg', { viewBox: '0 0 500 160', style: { width: '100%', height: '150px' } },
                      (() => {
                        const points = [
                          { year: '2022', fed: 4.25, cpi: 6.5 },
                          { year: '2023', fed: 5.50, cpi: 3.4 },
                          { year: '2024', fed: 5.25, cpi: 3.1 },
                          { year: '2025', fed: 4.75, cpi: 2.8 },
                          { year: '2026', fed: 4.25, cpi: 2.5 }
                        ];
                        const getX = (idx) => (idx / 4) * 440 + 30;
                        const getY = (val) => 140 - (val / 7.0) * 120;

                        const polyFed = points.map((p, i) => `${getX(i)},${getY(p.fed)}`).join(' ');
                        const polyCpi = points.map((p, i) => `${getX(i)},${getY(p.cpi)}`).join(' ');

                        return [
                          h('polyline', { key: 'fed', fill: 'none', stroke: '#10b981', strokeWidth: '3', points: polyFed }),
                          h('polyline', { key: 'cpi', fill: 'none', stroke: '#fb7185', strokeWidth: '3', strokeDasharray: '4,4', points: polyCpi }),
                          points.map((p, i) => h('text', { key: i, x: getX(i), y: 155, fill: 'var(--text-muted)', fontSize: '10', textAnchor: 'middle' }, p.year))
                        ];
                      })()
                    ),
                    h('p', { style: { fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.5rem', lineHeight: 1.4 } },
                      '隨著 CPI 通膨順利從 6.5% 回落至 3.0% 政策目標區間，美聯儲基準利率迎來降息轉折，提供科技股與股市資金評價提升水準。'
                    )
                  )
                ),

                // AI Macro Asset Allocation Strategy Assessment
                h('div', { style: { background: 'rgba(15, 23, 42, 0.9)', border: '1px solid var(--accent-emerald)', borderRadius: '12px', padding: '1.25rem' } },
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.85rem' } },
                    h('span', { style: { fontSize: '1.3rem' } }, '🤖'),
                    h('h3', { className: 'gradient-text', style: { fontSize: '1.05rem', fontWeight: 800 } }, 'StockMind AI 總體經濟與資產配置研判')
                  ),
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1rem', fontSize: '0.82rem', lineHeight: 1.6 } },
                    h('div', { style: { background: 'rgba(255,255,255,0.03)', padding: '1rem', borderRadius: '8px', borderLeft: '3px solid #10b981' } },
                      h('h4', { style: { color: '#34d399', fontWeight: 800, marginBottom: '0.3rem' } }, '🚀 股票與 ETF 戰略'),
                      h('p', { style: { color: '#cbd5e1' } }, '台灣與美國科技大廠受惠 AI 伺服器建置熱潮，建議核心配置仍以台積電 (2330)、AI 伺服器龍頭與市值型 ETF (0050) 為主，並搭配高股息 ETF 提供下行防禦。')
                    ),
                    h('div', { style: { background: 'rgba(255,255,255,0.03)', padding: '1rem', borderRadius: '8px', borderLeft: '3px solid #38bdf8' } },
                      h('h4', { style: { color: '#38bdf8', fontWeight: 800, marginBottom: '0.3rem' } }, '💵 債券與避險資產戰略'),
                      h('p', { style: { color: '#cbd5e1' } }, '在美聯儲降息趨勢下，長天期美國國債 ETF 具備鎖定高殖利率與資本利得爆發力；黃金現貨則在各國央行去美元化儲備需求下，為投資組合提供抗通膨避險。')
                    )
                  )
                )
              ) :

            // Main Layout Screener
            h('div', null,
              ...
              h('div', { className: 'glass-panel', style: { padding: '1.1rem 1.5rem', marginBottom: '1.5rem', background: 'linear-gradient(135deg, rgba(15, 23, 42, 0.9) 0%, rgba(30, 41, 59, 0.8) 100%)', border: '1px solid rgba(16, 185, 129, 0.3)' } },
                h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.75rem' } },
                  ...
                  h('div', { style: { display: 'flex', gap: '0.4rem', flexWrap: 'wrap' } },
                    [
                      ...
                      { id: 'qualityCompany', label: '🏆 長線獲利好公司', icon: '🏆' },
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '0.9rem' } },
                  getFeaturedHotStocks(currentStocks, activeHotTab).map(stock =>
                    h('div', {
                      key: stock.ticker,
                      className: `stock-card ${stock.isMarketOpen !== false ? (priceFlashMap[stock.ticker] || '') : ''}`,
                      style: {
              ...
              h('div', { className: `floating-chat-widget ${isChatMinimized ? 'minimized' : ''}` },
                h('div', {
                  ...
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.35rem' } },
                    h('button', {
                      onClick: (e) => { e.stopPropagation(); runPortfolioBacktest(activePortfolioItems, 3, portfolioBudget); },
                      style: { background: 'rgba(6, 182, 212, 0.25)', border: '1px solid rgba(6, 182, 212, 0.6)', color: '#38bdf8', padding: '0.2rem 0.5rem', borderRadius: '6px', fontSize: '0.72rem', fontWeight: 800, cursor: 'pointer' }
                    }, '📈 歷史回測'),
                    h('button', {
                      ...
                      h('div', { key: i, style: { alignSelf: m.sender === 'user' ? 'flex-end' : 'flex-start', maxWidth: '95%' } },
                        h('div', {
                          ...
                          m.portfolioCard && h('div', { style: { marginTop: '0.6rem', padding: '0.65rem', background: 'rgba(15,23,42,0.9)', border: '1px solid rgba(16,185,129,0.4)', borderRadius: '8px', fontSize: '0.75rem' } },
                            ...
                            h('button', {
                              onClick: () => runPortfolioBacktest(m.portfolioCard.items, 3, m.portfolioCard.budget),
                              className: 'btn-secondary',
                              style: { width: '100%', padding: '0.35rem', fontSize: '0.72rem', marginTop: '0.35rem', border: '1px solid var(--accent-emerald)', color: '#34d399', fontWeight: 800 }
                            }, '📈 一鍵歷史績效回測 (Backtest)')
                          ),
                          ...
                          m.topStocks && h('div', { style: { marginTop: '0.4rem', paddingTop: '0.35rem', borderTop: '1px solid rgba(255,255,255,0.1)', display: 'flex', gap: '0.3rem', flexWrap: 'wrap' } },
                            m.topStocks.map(s =>
                              ...
                              }, `🔍 ${s.name} (${s.ticker})${s.changePercent !== undefined ? ' • ' + (s.changePercent>=0?'+':'') + s.changePercent + '%' : (s.yield ? ' • 殖利率 ' + s.yield + '%' : '')}`)
                            )
                          )
                        )
                      )
              ...
              h('div', null,
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))', gap: '1rem' } },
                  displayedStocks.map(stock =>
                    h('div', { key: stock.ticker, className: `glass-panel-interactive ${stock.isMarketOpen !== false ? (priceFlashMap[stock.ticker] || '') : ''}`, style: { padding: '1.1rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', transition: 'all 0.3s' } },
                      h('div', null,
                        h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' } },
                          h('div', null,
                            ...
                            stock.isMarketOpen === false 
                              ? h('span', { className: 'pill-badge pill-purple', style: { marginLeft: '0.2rem', fontSize: '0.62rem', background: 'rgba(255,255,255,0.08)', color: 'var(--text-muted)' } }, '⏸️ 已收盤')
                              : h('span', { className: 'pill-badge pill-emerald', style: { marginLeft: '0.2rem', fontSize: '0.62rem' } }, '🟢 交易中'),
                            h('span', { className: 'pill-badge pill-gold', style: { marginLeft: '0.2rem', fontSize: '0.62rem', fontWeight: 800 } }, stock.currency === 'USD' ? '💵 USD 美元' : '💵 TWD 新台幣'),
                        ...
                        h('div', { style: { fontSize: '1.35rem', fontWeight: 800, margin: '0.6rem 0 0.2rem', fontFamily: 'var(--font-mono)' } },
                        ...
                        // Dynamic Intraday/Historical SVG Sparkline Trend Chart
                        h('div', { style: { height: '36px', margin: '0.4rem 0 0.6rem', width: '100%', background: 'rgba(15,23,42,0.5)', borderRadius: '6px', padding: '2px 4px', display: 'flex', alignItems: 'center' } },
                          (() => {
                            const hist = stock.history || [
                              stock.price * 0.96, stock.price * 0.97, stock.price * 0.965, stock.price * 0.98,
                              stock.price * 0.99, stock.price * 0.985, stock.price * 0.995, stock.price
                            ];
                            ...
                            const points = hist.map((v, i) => `${(i / (hist.length - 1)) * 260 + 5},${30 - ((v - minV) / range) * 24 + 3}`).join(' ');

                            return h('svg', { width: '100%', height: '100%', viewBox: '0 0 270 36', style: { overflow: 'visible' } },
                              h('polyline', {
                                fill: 'none',
                                stroke: strokeColor,
                                strokeWidth: '2.2',
                        ...
                        h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.4rem', padding: '0.5rem', background: 'rgba(15,23,42,0.7)', borderRadius: '6px', fontSize: '0.73rem', marginBottom: '0.6rem' } },
                          h('div', null, '幣值: ', h('strong', { style: { color: 'var(--accent-gold)' } }, stock.currency === 'USD' ? 'USD' : 'TWD')),
            ...
            isPortfolioOpen && h('div', { style: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(5,8,16,0.85)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1300, padding: '1rem' } },
              h('div', { className: 'glass-panel', style: { width: '100%', maxWidth: '960px', padding: '1.8rem', maxHeight: '92vh', overflowY: 'auto', borderRadius: '16px' } },
                ...
                // Footer Buttons
                h('div', { style: { marginTop: '1.5rem', display: 'flex', gap: '0.75rem', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' } },
                  h('div', { style: { display: 'flex', gap: '0.5rem', flexWrap: 'wrap' } },
                    ...
                    h('button', { onClick: () => runPortfolioBacktest(activePortfolioItems, 3, portfolioBudget), className: 'btn-primary', style: { padding: '0.55rem 1.2rem', fontSize: '0.85rem', background: 'linear-gradient(135deg, #06b6d4 0%, #3b82f6 100%)' } }, '📈 一鍵歷史績效回測 (Backtest)')
                  ),
            ...
            // Quant Backtest Interactive Visual Modal
            isBacktestModalOpen && h('div', { style: { position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(5,8,16,0.85)', backdropFilter: 'blur(10px)', zIndex: 1300, display: 'flex', alignItems: 'center', justifyContent: 'center' }, onClick: () => setIsBacktestModalOpen(false) },
              h('div', { className: 'glass-panel', style: { maxWidth: '850px', width: '92%', maxHeight: '90vh', overflowY: 'auto', padding: '1.5rem', background: '#090d16', border: '1px solid var(--accent-emerald)', boxShadow: '0 0 35px rgba(16,185,129,0.3)' }, onClick: (e) => e.stopPropagation() },
                
                // Header
                h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.2rem', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '0.75rem' } },
                  h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.6rem' } },
                    h('span', { style: { fontSize: '1.5rem' } }, '📈'),
                    h('div', null,
                      h('h3', { className: 'gradient-text', style: { fontSize: '1.2rem', fontWeight: 800 } }, 'StockMind AI 量化歷史回測與績效對比'),
                      h('p', { style: { fontSize: '0.75rem', color: 'var(--text-secondary)' } }, `對比近 ${backtestYears} 年歷史總報酬、CAGR、MDD最大回撤與元大台灣50/標普500超額報酬 (Alpha)`)
                    )
                  ),
                  h('button', { onClick: () => setIsBacktestModalOpen(false), style: { background: 'none', border: 'none', color: 'var(--text-secondary)', fontSize: '1.5rem', cursor: 'pointer' } }, '✕')
                ),

                // Interactive Backtest Settings Controls Bar
                h('div', { className: 'glass-panel', style: { padding: '1rem', marginBottom: '1.25rem', background: 'rgba(15,23,42,0.85)', border: '1px solid rgba(16,185,129,0.3)' } },
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: '0.85rem', marginBottom: '0.85rem' } },
                    
                    // 1. Time Horizon Switcher
                    h('div', null,
                      h('label', { style: { fontSize: '0.72rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.3rem' } }, '⏱️ 回測時間跨度'),
                      h('div', { style: { display: 'flex', gap: '0.25rem' } },
                        [1, 2, 3, 5].map(y =>
                          h('button', {
                            key: y,
                            onClick: () => runPortfolioBacktest(null, y),
                            style: {
                              flex: 1,
                              background: backtestYears === y ? 'var(--accent-emerald)' : 'rgba(255,255,255,0.06)',
                              color: backtestYears === y ? '#090d16' : 'var(--text-secondary)',
                              border: 'none', padding: '0.25rem 0.3rem', borderRadius: '6px', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer'
                            }
                          }, `${y}Y`)
                        )
                      )
                    ...
                    // 2. Investment Strategy (Lump-Sum vs Monthly DCA)
                    h('div', null,
                      h('label', { style: { fontSize: '0.72rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.3rem' } }, '💰 扣款投入策略'),
                      h('div', { style: { display: 'flex', gap: '0.25rem' } },
                        [
                          { id: 'lump_sum', label: '💰 單筆' },
                          { id: 'dca', label: '🔄 定期定額' }
                        ].map(st =>
                          h('button', {
                            key: st.id,
                            onClick: () => runPortfolioBacktest(null, null, null, st.id),
                            style: {
                              flex: 1,
                              background: backtestStrategy === st.id ? 'var(--accent-cyan)' : 'rgba(255,255,255,0.06)',
                              color: backtestStrategy === st.id ? '#090d16' : 'var(--text-secondary)',
                              border: 'none', padding: '0.25rem 0.3rem', borderRadius: '6px', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer'
                            }
                          }, st.label)
                        )
                    ...
                    // 3. Dividend DRIP Policy
                    h('div', null,
                      h('label', { style: { fontSize: '0.72rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.3rem' } }, '🔁 股利發放處理 (DRIP)'),
                      h('button', {
                        onClick: () => runPortfolioBacktest(null, null, null, null, null, !backtestDrip),
                        style: {
                          width: '100%',
                          background: backtestDrip ? 'rgba(245, 158, 11, 0.25)' : 'rgba(255,255,255,0.06)',
                          border: backtestDrip ? '1px solid #fbbf24' : '1px solid var(--border-subtle)',
                          color: backtestDrip ? '#fbbf24' : 'var(--text-secondary)',
                          padding: '0.25rem 0.5rem', borderRadius: '6px', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer'
                        }
                      }, backtestDrip ? '🔁 股利自動再投資 (DRIP)' : '💵 領取現金股利')
                    ),

                    // 4. Benchmark Selector
                    h('div', null,
                      h('label', { style: { fontSize: '0.72rem', color: 'var(--text-muted)', display: 'block', marginBottom: '0.3rem' } }, '🥊 對比基準標竿'),
                      h('select', {
                        value: backtestBenchmark,
                        onChange: (e) => runPortfolioBacktest(null, null, null, null, null, null, e.target.value),
                        style: {
                          width: '100%',
                          background: '#090d16',
                          border: '1px solid var(--border-subtle)',
                          color: '#34d399',
                          padding: '0.25rem 0.5rem', borderRadius: '6px', fontSize: '0.75rem', fontWeight: 700, outline: 'none', cursor: 'pointer'
                        }
                      },
                        h('option', { value: '0050' }, '🔵 0050 元大台灣50'),
                        h('option', { value: 'SPY' }, '🟣 SPY S&P500 標普500'),
                        h('option', { value: 'TLT' }, '🟡 TLT 美債與現金無風險')
                      )
                    )
                  ),

                  // Editable Investment Amounts Section
                  h('div', { style: { borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '0.75rem', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' } },
                    h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.5rem' } },
                      h('span', { style: { fontSize: '0.78rem', color: 'var(--text-secondary)', fontWeight: 700 } }, backtestStrategy === 'dca' ? '💵 初始投入本金 (NT$):' : '💵 投資總預算 (NT$):'),
                      h('input', {
                        type: 'number',
                        value: backtestBudget,
                        onChange: (e) => setBacktestBudget(Number(e.target.value)),
                        onBlur: () => runPortfolioBacktest(null, null, backtestBudget),
                        onKeyDown: (e) => { if (e.key === 'Enter') runPortfolioBacktest(null, null, backtestBudget); },
                        style: { background: '#090d16', border: '1px solid var(--accent-emerald)', color: '#34d399', borderRadius: '6px', padding: '0.25rem 0.5rem', fontSize: '0.82rem', fontWeight: 800, width: '130px', outline: 'none' }
                      }),
                      [500000, 1000000, 2000000].map(amt =>
                        h('button', {
                          key: amt,
                          onClick: () => runPortfolioBacktest(null, null, amt),
                          style: { background: 'rgba(255,255,255,0.06)', border: 'none', color: 'var(--text-secondary)', padding: '0.2rem 0.45rem', borderRadius: '4px', fontSize: '0.72rem', cursor: 'pointer' }
                        }, `${amt/10000}萬`)
                      )
                    ),

                    backtestStrategy === 'dca' && h('div', { style: { display: 'flex', alignItems: 'center', gap: '0.5rem' } },
                      h('span', { style: { fontSize: '0.78rem', color: 'var(--text-secondary)', fontWeight: 700 } }, '🔄 每月定期定額 (NT$):'),
                      h('input', {
                        type: 'number',
                        value: backtestMonthlyAmount,
                        onChange: (e) => setBacktestMonthlyAmount(Number(e.target.value)),
                        onBlur: () => runPortfolioBacktest(null, null, null, null, backtestMonthlyAmount),
                        onKeyDown: (e) => { if (e.key === 'Enter') runPortfolioBacktest(null, null, null, null, backtestMonthlyAmount); },
                        style: { background: '#090d16', border: '1px solid var(--accent-cyan)', color: '#38bdf8', borderRadius: '6px', padding: '0.25rem 0.5rem', fontSize: '0.82rem', fontWeight: 800, width: '110px', outline: 'none' }
                      }),
                      [5000, 10000, 20000, 50000].map(amt =>
                        h('button', {
                          key: amt,
                          onClick: () => runPortfolioBacktest(null, null, null, null, amt),
                          style: { background: 'rgba(255,255,255,0.06)', border: 'none', color: 'var(--text-secondary)', padding: '0.2rem 0.45rem', borderRadius: '4px', fontSize: '0.72rem', cursor: 'pointer' }
                        }, `${amt >= 10000 ? amt/10000+'萬' : amt/1000+'千'}`)
                      )
                    )
                  )
                ),
                ...
                // Metric Cards Grid
                backtestResult && h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '0.75rem', marginBottom: '1.5rem' } },
                  h('div', { style: { background: 'rgba(16, 185, 129, 0.15)', border: '1px solid rgba(16, 185, 129, 0.4)', padding: '0.8rem', borderRadius: '10px', textAlign: 'center' } },
                    h('div', { style: { fontSize: '0.7rem', color: '#34d399' } }, '💵 期末總資產'),
                    h('div', { style: { fontSize: '1.25rem', fontWeight: 900, color: '#34d399', marginTop: '2px' } }, `NT$ ${formatNum(backtestResult.finalPortfolioValue)}`)
                  ),
                  h('div', { style: { background: 'rgba(6, 182, 212, 0.15)', border: '1px solid rgba(6, 182, 212, 0.4)', padding: '0.8rem', borderRadius: '10px', textAlign: 'center' } },
                    h('div', { style: { fontSize: '0.7rem', color: '#38bdf8' } }, '📈 淨獲利金額 (獲利率)'),
                    h('div', { style: { fontSize: '1.15rem', fontWeight: 900, color: '#38bdf8', marginTop: '2px' } }, `+NT$ ${formatNum(backtestResult.netProfit !== undefined ? backtestResult.netProfit : (backtestResult.finalPortfolioValue - backtestResult.totalInvested))} (${backtestResult.totalReturnPct > 0 ? '+' : ''}${backtestResult.totalReturnPct}%)`)
                  ),
                  h('div', { style: { background: 'rgba(168, 85, 247, 0.15)', border: '1px solid rgba(168, 85, 247, 0.4)', padding: '0.8rem', borderRadius: '10px', textAlign: 'center' } },
                    h('div', { style: { fontSize: '0.7rem', color: '#c084fc' } }, '🚀 年化複合成長 (CAGR)'),
                    h('div', { style: { fontSize: '1.25rem', fontWeight: 900, color: '#c084fc', marginTop: '2px' } }, `+${backtestResult.cagr}% / 年`)
                  ),
                  h('div', { style: { background: 'rgba(245, 158, 11, 0.15)', border: '1px solid rgba(245, 158, 11, 0.4)', padding: '0.8rem', borderRadius: '10px', textAlign: 'center' } },
                    h('div', { style: { fontSize: '0.7rem', color: '#fbbf24' } }, '💰 歷史累計領取股息'),
                    h('div', { style: { fontSize: '1.1rem', fontWeight: 900, color: '#fbbf24', marginTop: '2px' } }, `NT$ ${formatNum(backtestResult.totalDividends)}`)
                  ),
                  h('div', { style: { background: 'rgba(244, 63, 94, 0.15)', border: '1px solid rgba(244, 63, 94, 0.4)', padding: '0.8rem', borderRadius: '10px', textAlign: 'center' } },
                    h('div', { style: { fontSize: '0.7rem', color: '#fb7185' } }, '🛡️ 最大歷史回撤 (MDD)'),
                    h('div', { style: { fontSize: '1.25rem', fontWeight: 900, color: '#fb7185', marginTop: '2px' } }, `-${backtestResult.mddPct}%`)
                  ),
                  h('div', { style: { background: 'rgba(99, 102, 241, 0.15)', border: '1px solid rgba(99, 102, 241, 0.4)', padding: '0.8rem', borderRadius: '10px', textAlign: 'center' } },
                    h('div', { style: { fontSize: '0.7rem', color: '#818cf8' } }, '⚡ 夏普比率 (Sharpe)'),
                    h('div', { style: { fontSize: '1.25rem', fontWeight: 900, color: '#818cf8', marginTop: '2px' } }, backtestResult.sharpe)
                  )
                ),

                // SVG Growth Line Chart
                backtestResult && backtestResult.chartPoints && h('div', { className: 'glass-panel', style: { padding: '1.25rem', marginBottom: '1.25rem', background: 'rgba(15, 23, 42, 0.85)', border: '1px solid rgba(16,185,129,0.3)' } },
                  h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' } },
                    h('h4', { style: { fontSize: '0.88rem', fontWeight: 800, color: '#fff' } }, `📊 ${backtestResult.targetName || '標的'} 淨值成長曲線對比 (Portfolio Net Asset Value vs. Benchmark)`),
                    h('div', { style: { display: 'flex', gap: '0.75rem', fontSize: '0.72rem', flexWrap: 'wrap' } },
                      h('span', { style: { color: '#10b981', fontWeight: 800 } }, `🟢 ${backtestResult.targetName || '目標標的'} (+${backtestResult.totalReturnPct || 0}%)`),
                      h('span', { style: { color: '#06b6d4', fontWeight: 800 } }, `🔵 基準大盤 (${backtestResult.benchmark || '0050'}) (+${backtestResult.benchmarkReturnPct || 0}%)`),
                      h('span', { style: { color: '#fbbf24', fontWeight: 800 } }, `🟡 累計投入本金 (NT$ ${formatNum(backtestResult.totalInvested || 0)})`)
                    )
                  ),
                  h('svg', { viewBox: '0 0 700 220', style: { width: '100%', height: '200px', overflow: 'visible' } },
                    (() => {
                      const points = backtestResult.chartPoints;
                      if (!points || points.length < 2) return null;

                      const pfVals = points.map(p => Number(p.portfolio) || 0);
                      const bmVals = points.map(p => Number(p.benchmarkVal) || 0);
                      const invVals = points.map(p => Number(p.invested) || 0);

                      const maxVal = Math.max(...pfVals, ...bmVals, ...invVals) * 1.05;
                      const minVal = Math.min(...pfVals, ...bmVals, ...invVals) * 0.95;
                      const range = maxVal - minVal || 1;

                      const getX = (idx) => (idx / (points.length - 1)) * 660 + 20;
                      const getY = (val) => 190 - ((val - minVal) / range) * 170;

                      const polyPortfolio = points.map((p, i) => `${getX(i)},${getY(p.portfolio || 0)}`).join(' ');
                      const polyBm = points.map((p, i) => `${getX(i)},${getY(p.benchmarkVal || 0)}`).join(' ');
                      const polyInv = points.map((p, i) => `${getX(i)},${getY(p.invested || 0)}`).join(' ');

                      const gridLines = [0.25, 0.5, 0.75].map(ratio => {
                        const yVal = 190 - ratio * 170;
                        return h('line', { key: ratio, x1: 20, y1: yVal, x2: 680, y2: yVal, stroke: 'rgba(255,255,255,0.08)', strokeDasharray: '2,2' });
                      });

                      return [
                        ...gridLines,
                        h('polyline', { key: 'inv', fill: 'none', stroke: '#fbbf24', strokeWidth: '2', strokeDasharray: '3,3', points: polyInv }),
                        h('polyline', { key: 'bm', fill: 'none', stroke: '#06b6d4', strokeWidth: '2.5', strokeDasharray: '4,4', points: polyBm }),
                        h('polyline', { key: 'pf', fill: 'none', stroke: '#10b981', strokeWidth: '3.5', points: polyPortfolio }),
                        points.filter((_, i) => i === 0 || i === points.length - 1 || i % 12 === 0).map((p, i) =>
                          h('circle', {
                            key: i,
                            cx: getX(p.month),
                            cy: getY(p.portfolio || 0),
                            r: 4,
                            fill: '#10b981',
                            stroke: '#090d16',
                            strokeWidth: 2
                ...
                // Footer Actions
                h('div', { style: { textAlign: 'center', marginTop: '1.25rem' } },
                  h('button', { onClick: () => setIsBacktestModalOpen(false), className: 'btn-primary', style: { padding: '0.55rem 2.5rem', fontSize: '0.85rem' } }, '完成查看回測報告')
                )
              )
            ),
                ...
                h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', textAlign: 'center' } },
                  ...
                  h('tbody', null,
                    h('tr', { style: { borderBottom: '1px solid var(--border-subtle)' } },
                      h('td', { style: { textAlign: 'left', padding: '0.6rem' } }, '💵 計價貨幣'),
                      compareList.map(s => h('td', { key: s.ticker, style: { color: 'var(--accent-gold)', fontWeight: 800 } }, s.currency === 'USD' ? '🇺🇸 USD (美元)' : '🇹🇼 TWD (新台幣)'))
                    ),
                    h('tr', { style: { borderBottom: '1px solid var(--border-subtle)' } },
              ...
              h('div', { className: 'glass-panel', style: { width: '100%', maxWidth: '850px', padding: '1.75rem', maxHeight: '92vh', overflowY: 'auto', borderRadius: '16px', background: 'linear-gradient(135deg, rgba(15, 23, 42, 0.95) 0%, rgba(30, 41, 59, 0.95) 100%)', border: '1px solid rgba(56, 189, 248, 0.4)', boxShadow: '0 25px 60px rgba(0,0,0,0.7)' } },
                ...
                h('div', { style: { marginBottom: '1.5rem' } },
                  ...
                  h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.65rem' } },
                    [
                      { label: '交易貨幣 (Currency)', val: selectedStock.currency === 'USD' ? 'USD 美元' : 'TWD 新台幣', sub: selectedStock.currency === 'USD' ? '🇺🇸 美國證券' : '🇹🇼 台灣證券', color: '#fbbf24' },
                ...
                h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1.25rem', marginBottom: '1.5rem' } },
                  ...
                  h('div', { className: 'glass-panel', style: { padding: '1rem' } },
                    h('h4', { style: { fontSize: '0.85rem', fontWeight: 800, color: 'var(--accent-purple)', marginBottom: '0.75rem' } }, '🔍 長線好公司與 20+ 指標即時檢核：'),
                    h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.45rem', fontSize: '0.75rem' } },
                      [
                        { label: 'ROE ≥ 10% (獲利品質)', pass: selectedStock.roe >= 10.0 },
                        { label: '自由現金流 (FCF) 為正', pass: true },
                        { label: '營業利益穩定成長', pass: true },
                        { label: '階梯式配息 (殖利率≥3%)', pass: selectedStock.yield >= 3.0 },
                        { label: '均線多頭排列 (5>20>60MA)', pass: selectedStock.maAlign !== false },
                        { label: '突破 60日季線生命線', pass: selectedStock.breakMa60 !== false },
                        { label: '成交量爆發 2倍以上', pass: selectedStock.volumeBurst !== false },
                        { label: '三大法人同買超挺進', pass: selectedStock.institutionalBuy !== false }
                      ].map((chk, idx) =>
                    ...
                    h('button', {
                      onClick: () => {
                        if (selectedStock) {
                          ...
                          runPortfolioBacktest([target], 3, 1000000);
                        }
                      },
                      className: 'btn-primary',
                      style: { fontSize: '0.8rem', padding: '0.45rem 1rem', background: 'linear-gradient(135deg, #06b6d4 0%, #3b82f6 100%)' }
                    }, '📈 個股歷史回測 (Backtest)'),
                    h('button', {
                      onClick: () => {
                        if (selectedStock) {
            ...
            // Mobile Native Bottom Navigation Bar (手機專屬原生底部導覽列)
            h('div', { className: 'mobile-bottom-nav' },
              h('button', {
                className: `mobile-nav-btn ${activeView === 'screener' ? 'active' : ''}`,
                onClick: () => setActiveView('screener')
              },
                h('span', { style: { fontSize: '1.2rem' } }, '📊'),
                h('span', null, '篩選主控')
              ...
              h('button', {
                className: `mobile-nav-btn ${activeView === 'macro' ? 'active' : ''}`,
                onClick: () => setActiveView('macro')
              },
                h('span', { style: { fontSize: '1.2rem' } }, '🌐'),
                h('span', null, '全球總經')
              ...
              h('button', {
                ...
                h('span', { style: { fontSize: '1.2rem' } }, '💬'),
                h('span', null, isChatMinimized ? '開啟AI' : '縮小AI')
              ),
              h('button', {
                className: 'mobile-nav-btn',
                ...
                h('span', { style: { fontSize: '1.2rem' } }, '⚖️'),
                h('span', null, `對比(${compareList.length})`)
              ),
              h('button', {
                className: 'mobile-nav-btn',
                ...
                h('span', { style: { fontSize: '1.2rem' } }, '⭐'),
                h('span', null, `觀察(${watchlist.length})`)
              )
            )
          );
        }
...
print('Cleaned Stock Screener & Portfolio App index.html generated successfully!')
